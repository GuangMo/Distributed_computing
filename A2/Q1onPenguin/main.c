...
#include "mpi.h"
#include <assert.h>
#define INTERVAL_FACTOR 500 //This means we will devide the range into 50 sub ranges
#define TERMINATE_SIGNAL 1 //To signal work is done

long long int findNextPrime( long long int curNum );
long long int findLargestPrimeGap( long long int beginVal, long long int endVal);
long long int findLargestPrimeGapLast( long long int beginVal, long long int endVal);
...
int main(int argc, char* argv[])
{
       ...
       int      numProcess;
       long long int beginValue, endValue; // variables that stored the end values of the range
    ...
    if (numProcess != 1)
    {
       ...
       if( my_rank == 0 )
       {
           ...
           long long int eachProRange = ((endValue - beginValue)/INTERVAL_FACTOR)/(numProcess);
           ...
           long long int * rangeArray = (long long int * ) Malloc( totalPartitions+1, sizeof(long long int ), MPI_COMM_WORLD, my_rank );
           int revBuffer[(numProcess -1)]; //buffer that main process use to collect max gap from slave
           long long int sendBuffer[2]; //contains the boundaries of a range
           ...
           for( i = 1; i < numProcess; i++)
           {
               ...
               MPI_Isend(&sendBuffer, 2, MPI_LONG_LONG, i, tag, MPI_COMM_WORLD, &reqs[i] );
       ...
       else  // other process
       {
                ...
                long long int subInterval[2];
                int flag=0;
                MPI_Status stat;
                MPI_Request req;
                int done = 0;
                MPI_Irecv( &subInterval, 2, MPI_LONG_LONG, 0, tag, MPI_COMM_WORLD, &req);
                int localMaxGap = 0;
                int processMaxGap = 0;
                int callCount =0; //gmo
                while ( !done)
...
long long int findLargestPrimeGap( long long int beginVal, long long int endVal)
{
        int maxGap = 0;
        int diff;
        long long int curPrime, nextPrime;
